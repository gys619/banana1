...
默认按账号顺序提交
如需自定义请在环境变量[jd_wealth_island_codeId](只提交前3个)例:
jd_wealth_island_codeId="3,5,8"
已支持IOS双京东账号,Node.js支持N个京东账号
脚本兼容: QuantumultX, Surge, Loon, JSBox, Node.js
============Quantumultx===============
[task_local]
#京喜财富岛互助
18 0,18 * * * https://raw.githubusercontent.com/444444/JDJB/main/jd_cfd_help.js, tag=京喜财富岛互助, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jxcfd.png, enabled=true

================Loon==============
[Script]
cron "18 0,18 * * *" script-path=https://raw.githubusercontent.com/444444/JDJB/main/jd_cfd_help.js,tag=京喜财富岛互助

===============Surge=================
京喜财富岛互助 = type=cron,cronexp="18 0,18 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/444444/JDJB/main/jd_cfd_help.js

============小火箭=========
京喜财富岛互助 = type=cron,script-path=https://raw.githubusercontent.com/444444/JDJB/main/jd_cfd_help.js, cronexpr="18 0,18 * * *", timeout=3600, enable=true
...
let codeIndex = $.getval('jd_wealth_island_codeId') || '' // 定义提交助力码的账号如2,3,5
if ($.isNode() && process.env.jd_wealth_island_codeId) {
  codeIndex = process.env.jd_wealth_island_codeId;
}
let codeIndexArr = []
...
!(async () => {
  ...
  let getShareNums = 0
  if (HelpAuthorFlag) {
    $.innerInviteList = await getAuthorShareCode('https://gitee.com/444444521/JD-Scripts/raw/master/shareCodes/cfd.json');
    if(!$.innerInviteList[0]) $.innerInviteList = await getAuthorShareCode('https://gitee.com/444444521/JD-Scripts/raw/master/shareCodes/cfd.json');
    res2 = await getAuthorShareCode('https://gitee.com/444444521/JD-Scripts/raw/master/shareCodes/cfd.json');
    if(!res2[0]) res2 = await getAuthorShareCode('https://gitee.com/444444521/JD-Scripts/raw/master/shareCodes/cfd.json');
...
function getCode(){
  if(!$.getval('jd_wealth_island_code') || ($.getval('jd_wealth_island_code') && !$.getval('jd_wealth_island_code')[$.time('yyyyMMd')])){
    $.jd_wealth_island_code = {}
    $.jd_wealth_island_code[$.time('yyyyMMd')] = []
    $.setdata($.jd_wealth_island_code,'jd_wealth_island_code')
  }else{
    $.jd_wealth_island_code = $.getval('jd_wealth_island_code')
  }
  if(!codeIndex){
