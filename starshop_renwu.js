...
$.linkID = '';
let uniqueIdList = [
    {'id':'GV949A','name':'肖战','linkID':'nUW_swWdzGTw9TREVAMBCw','taskId':""},
];
const rewardList = ['nUW_swWdzGTw9TREVAMBCw']
if ($.isNode()) {
...
async function takePostRequest(type) {
    ...
    switch (type) {
        case 'activityStarBackGetProgressBarInfo':
            body = `functionId=starShopPageInfo&body={"taskId":"","encryptionPin":"","linkId":"nUW_swWdzGTw9TREVAMBCw"}&_t=${Date.now()}&appid=activities_platform`;
            myRequest = getPostRequest(body);
            console.log(body);
            break;
        case 'apTaskList':
            body = `functionId=apTaskList&body={"linkId":"nUW_swWdzGTw9TREVAMBCw"}&_t=${Date.now()}&appid=activities_platform`;
            myRequest = getPostRequest(body);
            break;
        case 'SIGN':
            ...
            break;
        case 'apTaskDetail':
            body = `functionId=apTaskDetail&body={"taskType":"BROWSE_CHANNEL","taskId":${$.oneTask.id},"channel":4,"linkId":"nUW_swWdzGTw9TREVAMBCw"}&_t=${Date.now()}&appid=activities_platform`;
            myRequest = getPostRequest(body);
            break;
        case 'apDoTask':
            body = `functionId=apDoTask&body={"taskType":"BROWSE_CHANNEL","taskId":${$.oneTask.id},"channel":4,"linkId":"nUW_swWdzGTw9TREVAMBCw","itemId":"${encodeURIComponent($.oneItemInfo.itemId)}"}&_t=${Date.now()}&appid=activities_platform`;
            myRequest = getPostRequest(body);
            break;
...
async function getInfo() {
    const url = `https://api.m.jd.com/?functionId=starShopPageInfo&body={%22taskId%22:%22${"nUW_swWdzGTw9TREVAMBCw"}%22,%22linkId%22:%22${$.linkID}%22}&_t=${Date.now()}&appid=activities_platform`;
    const headers = {
        'Origin' : `https://prodev.m.jd.com`,
        'Cookie': $.cookie,
        'Connection' : `keep-alive`,
        'Accept' : `application/json, text/plain, */*`,
        'Referer' : `https://prodev.m.jd.com/mall/active/3GpvvutHU8PsMmvTXqYya3PQNTG3/index.html`,
        'Host' : `api.m.jd.com`,
        'User-Agent': $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : (require('./USER_AGENTS').USER_AGENT)) : ($.getdata('JDUA') ? $.getdata('JDUA') : "jdapp;iPhone;9.4.4;14.3;network/4g;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1"),
        'Accept-Encoding' : `gzip, deflate, br`,
        'Accept-Language' : `zh-cn`
