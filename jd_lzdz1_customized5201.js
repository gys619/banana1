...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      authorCodeList = [
        "f2435f5b55a1436589aef8e25e12b0f7",
        "d9ff9dd677194cf0aa85006da14ce06f",
        "58ff4eb11f204bf6ba3b0a8a7522f8c4"
      ];
      ...
      $.activityShopId = "1000075555";
      $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/may/childrenWear/activity/${$.authorNum}?activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&adsource=null&shareuserid4minipg=${encodeURIComponent($.secretPin)}&shopid=${$.activityShopId}&sid=&un_area=`;
      await member();
      await $.wait(1000);
      if ($.bean > 0) {
...
async function member() {
  ...
  if ($.token) {
    ...
    if ($.secretPin) {
      ...
      if ($.index === 1) {
        await task("/may/childrenWear/activityContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&shareUuid=${encodeURIComponent($.authorCode)}`, 0, 1);
      } else {
        await task("/may/childrenWear/activityContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&shareUuid=${encodeURIComponent($.authorCode)}`);
      }
      $.log("关注店铺");
      await task("may/childrenWear/saveTask", `activityId=${$.activityId}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.secretPin)}&taskType=23&taskValue=23&shareUuid=${encodeURIComponent($.authorCode)}`);
      await task("may/childrenWear/saveTask", `activityId=${$.activityId}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.secretPin)}&taskType=21&taskValue=&shareUuid=${encodeURIComponent($.authorCode)}`);
      await task("taskact/common/drawContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}`);
      await task("may/childrenWear/initOpenCard", `pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}`);
      $.log("加入店铺会员");
      if ($.openCardList) {
      ...
      await task("may/childrenWear/saveTask", `activityId=${$.activityId}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.secretPin)}&taskType=21&taskValue=21&shareUuid=${encodeURIComponent($.authorCode)}`);
            ...
            if (data.result) {
              switch (function_id) {
                ...
                case "wxActionCommon/getUserInfo":
                  break;
                case "/may/childrenWear/activityContent":
                  if (!data.data.hasEnd) {
                  ...
                  break;
                case "may/childrenWear/initOpenCard":
                  $.openCardList = data.data.openInfo;
                  $.openCardStatus = data.data.allOpenCard;
                  // console.log(data)
                  break;
                case "may/childrenWear/saveTask":
                  console.log(data);
                  break;
                case "may/childrenWear/draw":
                  console.log(data);
                  break;
