/*
7.24-8.4 甜蜜七夕跨越时空中意你
第一个账号助力作者 其他依次助力CK1
第一个CK失效会退出脚本
————————————————
入口：[ 7.24-8.4 甜蜜七夕跨越时空中意你 (https://lzdz1-isv.isvjcloud.com/dingzhi/jul/qixiFirst/activity?activityId=dz46b2dbbde1914438a47a994cc11c)]

请求太频繁会被黑ip
过10分钟再执行

0 8 24 7 * jd_opencard53.js
*/
...
opencard_addSku = process.env.opencard_addSku_All ? process.env.opencard_addSku_All : `${opencard_addSku}` ;
opencard_draw = process.env.opencard_draw ? process.env.opencard_draw : opencard_draw ;
allMessage = ""
message = ""
$.hotFlag = false
$.outFlag = false
...
!(async () => {
  ...
  $.shareUuid = "1d82de79042d4dc3ad1ef9fb9dd5f33f"
  console.log(`入口:\nhttps://lzdz1-isv.isvjcloud.com/dingzhi/jul/qixiFirst/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`)
  let shareUuidArr = ['1d82de79042d4dc3ad1ef9fb9dd5f33f','8df4f3475b324f56b0ade8b68c0382e3']
  let n = 0
  ...
  try {
    ...
    if(!$.followSku && !$.outFlag){
      if(opencard_addSku+"" == "true"){
        ...
        console.log('如需加购请设置变量[opencard_addSku_All]为"true"');
      }
    }
    if(flag){
      await takePostRequest('activityContent');
    ...
    console.log(`${$.score}值`)
    if(opencard_draw+"" !== "0" && $.joinShopStatus){
      $.runFalag = true
  ...
  switch (type) {
    case 'isvObfuscator':
      ...
      case 'accessLogWithAD':
        ...
        let pageurl = `${domain}/dingzhi/jul/qixiFirst/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`
      ...
      case 'activityContent':
        url = `${domain}/dingzhi/jul/qixiFirst/activityContent`;
      ...
      case 'drawContent':
        ...
        break;
      case 'checkOpenCard':
        url = `${domain}/dingzhi/jul/qixiFirst/initOpenCard`;
      ...
      case 'startDraw':
        ...
        break;
      case 'followShop':
        url = `${domain}/dingzhi/jul/qixiFirst/saveTask`;
      ...
      case 'getShareRecord':
        ...
        break;
      case '抽奖':
        url = `${domain}/dingzhi/jul/qixiFirst/draw`;
...
function getPostRequest(url, body, method="POST") {
  ...
  if(url.indexOf('https://lzdz1-isv.isvjcloud.com') > -1){
    headers["Referer"] = `https://lzdz1-isv.isvjcloud.com/dingzhi/jul/qixiFirst/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`
...
function getCk() {
  return new Promise(resolve => {
    let get = {
      url:`https://lzdz1-isv.isvjcloud.com/dingzhi/jul/qixiFirst/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`,
