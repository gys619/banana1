...
[task_local]
#赚京豆-瓜分京豆
10 0,17 * * * jd_syj.js, tag=赚京豆-瓜分京豆, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jd_syj.png, enabled=true

================Loon==============
[Script]
cron "10 0,17 * * *" script-path=jd_syj.js, tag=赚京豆-瓜分京豆

===============Surge=================
赚京豆-瓜分京豆 = type=cron,cronexp="10 0,17 * * *",wake-system=1,timeout=3600,script-path=jd_syj.js

============小火箭=========
赚京豆-瓜分京豆 = type=cron,script-path=jd_syj.js, cronexpr="10 0,17 * * *", timeout=3600, enable=true
 */
          ...
          if (cookiesArr[i]) {
            ...
            if ((cookiesArr.length > $.assistNum)) {
              ...
              for (let j = 0; j < $.tuanList.length; ++j) {
                ...
                if($.canHelpSuccess){
                  console.log(`账号 ${$.UserName} 开始给 【${$.tuanList[j]['assistedPinEncrypted']}】助力成功\n`)
                }
              }
              if(!$.canHelp) break
              await $.wait(3000)
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.success) {
              ...
              if (data.resultCode === '2400205') {console.log('助力结果：团已满\n');$.canHelp = false}
            }
          }
        }
      } catch (e) {
