...
!(async () => {
    ...
    // authorCodeList = await getAuthorCodeList('https://gitee.com/fatelight/code/raw/master/lzdz1_21.json')
    // if($.getAuthorCodeListerr === false){
    //     authorCodeList = [
    //         '5f037ae29a6447568f57d2a2a9e3e49c',
    //     ]
    // }
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            authorCodeList = [
                'b3e8333b8de547ec9d6722f3c2612420',
                '3ed172ca3b8b40cd9470d539d51389f1',
                '84b58e2402224842b120fac96ad33942',
            ]
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            if ($.openCardStatus) {
                ...
                await task('dz/openCard/checkOpenCard', `activityId=${$.activityId}&actorUuid=${$.actorUuid}&shareUuid=${$.authorCode}&pin=${encodeURIComponent($.secretPin)}`)
                await $.wait(1000)
                await task('dz/openCard/saveTask', `activityId=${$.activityId}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.secretPin)}&taskType=2&taskValue=100006950197`)
                // await $.wait(2000)
