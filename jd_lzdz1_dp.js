...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            authorCodeList = [
                '73d4d90905964849a45d82375c5595cb',
                '8c74bab98a5249ee9a52a410322f2fdc',
                'a9561a97904b46ec86dc4ac5cc249581',
                '2af6be6d079a4513aade9a3a252a884f',
            ]
            $.bean = 0;
            ...
            $.activityShopId = '1000204895'
            $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/bigname/memberday/activity/${$.authorNum}?activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&adsource=null&shareuserid4minipg=null&shopid=${$.activityShopId}&lng=00.000000&lat=00.000000&sid=&un_area=`
            await opencard05();
            await $.wait(2000)
            await opencard05();
            if ($.bean > 0) {
...
async function opencard05() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            await task('bigname/memberday/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=${encodeURIComponent($.pinImg)}&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}`)
            await task("taskact/common/drawContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}`)
            if ($.activityContent) {
                await task('bigname/memberday/initOpenCard', `activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&pin=${encodeURIComponent($.secretPin)}`)
                $.log("\n关注店铺")
                await task('bigname/memberday/saveTask', `activityId=${$.activityId}&actorUuid=${$.actorUuid}&shareUuid=${$.authorCode}&pin=${encodeURIComponent($.secretPin)}&taskType=23&taskValue=1&shareUuid=${$.authorCode}`)
                $.log("\n加入店铺会员");
                if (!$.openCardStatus) {
                    $.log("模拟上报访问记录")
                ...
                for (let index = 0; index < $.score; index++) {
                    await $.wait(2000)
                    await getFirstLZCK()
                    await task('bigname/memberday/draw', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}`)
                    
                }
                // await $.wait(1000)
                $.log("\n逛会场")
                await task('bigname/memberday/saveTask', `activityId=${$.activityId}&actorUuid=${$.actorUuid}&shareUuid=${$.authorCode}&pin=${encodeURIComponent($.secretPin)}&taskType=12&taskValue=3&shareUuid=${$.authorCode}`)
            } else {
                $.log("无法顺利的获取到活动信息。")
            }
        } else {
            ...
            try {
                if (err) {
                    ...
                    if (data) {
                        ...
                        if (data.result) {
                            switch (function_id) {
                                case 'bigname/memberday/draw':
                                    if (data.data) {
                                        console.log(data.data);
                                    }
                                    break;
                                case 'bigname/memberday/stopPlayGame':
                                    console.log(data.data.res);
                                    break;
                                case 'bigname/memberday/playGame':
                                    $.gameId = data.data
                                    break;
                                ...
                                case 'wxActionCommon/getUserInfo':
                                    ...
                                    break;
                                case 'bigname/memberday/activityContent':
                                    if (!data.data.hasEnd) {
