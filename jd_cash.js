...
const inviteCodes = []
let signUrl = process.env.signUrl ?? "http://106.126.11.114:5555/jdsign";
let jdPandaToken = '';
jdPandaToken = $.isNode() ? (process.env.jdPandaToken ? process.env.jdPandaToken : `${jdPandaToken}`) : ($.getdata('gua_cleancart_PandaToken') ? $.getdata('gua_cleancart_PandaToken') : `${jdPandaToken}`);
if (!jdPandaToken && !signUrl) {
  console.log('请填写Panda获取的Token变量为:jdPandaToken或者填写自建signUrl变量为:signUrl');
  return;
}
if ($.isNode()) {
...
async function appdoTask(type,taskInfo) {
  ...
  let sign = await getSignOpr(functionId, body)
  return new Promise((resolve) => {
...
function getSignOpr(functionId, body){
  if(jdPandaToken){
    console.log("jdPandaToken不为空时,优先panda接口");
    return getSignfromPanda(functionId, body);
  }
  if (signUrl){
    return getSign(functionId, body);
  }
}
