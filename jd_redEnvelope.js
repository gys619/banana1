...
const Faker = true//$.isNode() ? require('./sign_graphics_validate.js') : '';
...
async function run() {
  try {
    ...
    if ($.index == 1 && t == 1) {
      await $.wait(parseInt(Math.random() * 2000 + 100, 10))
...
function getUA() {
  ...
  rebateCode = 'SKMR7Gw'
  if ($.index != 1) {
    let arr = [rebateCodes, 'SKMR7Gw']
    rebateCode = arr[Math.floor(Math.random() * arr.length)] || rebateCode
    if (!rebateCode) rebateCode = 'SKMR7Gw'
  }
