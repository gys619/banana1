          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              ...
              if (data.factoryList && data.productionList) {
                ...
                // 报告运行次数
                $.get({
                  url: `https://123/api/runTimes?activityId=jxdfactory&sharecode=${data.user.encryptPin}`,
                  timeout: 10000
                }, (err, resp, data) => {
                  if (err) {
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `https://123/api/jxdfactory/${randomCount}`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
