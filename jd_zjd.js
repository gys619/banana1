...
[task_local]
#赚京豆
0 0-23/3 * * * https://raw.githubusercontent.com/11111115/JDHelp/main/jd_zjd.js, tag=赚京豆, enabled=true
*/
const $ = new Env('赚京豆');
    ...
    $.get(options, async (err, resp, data) => {
      try {
        if (err) {
            ...
            data = JSON.parse(data);
            for (let i = 0; i < data.zjd.length; i++) {
                let code = JSON.parse(Base64.decode(data.zjd[i]))
                $.authorTuanList.push(code);
            }
        }
      } catch (e) {
        $.logErr(e, resp)
