/*
泉粉
10:/邀请好友成为碧欧泉会员，免费拿500元泉能好礼！升级新版倞マDoσσng ！J3pcr6stGqQH！
*/
const $ = new Env("泉粉");
...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            authorCodeList = [
                'a8e3eebf11be4b44869698f1e767e877',
                '9f0242b7068443e7a947170c8e282204'
            ]
            // $.authorCode = authorCodeList[random(0, authorCodeList.length)]
            $.authorCode = ownCode ? ownCode : authorCodeList[random(0, authorCodeList.length)]
            $.authorNum = `${random(1000000, 9999999)}`
            $.randomCode = random(1000000, 9999999)
            $.activityId = 'dz2009100009970144'
            $.activityShopId = '1000099701'
            $.activityUrl = `https://lzdz-isv.isvjcloud.com/dingzhi/biouquan/activity/${$.authorNum}?activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&adsource=null&shareuserid4minipg=${encodeURIComponent($.secretPin)}&shopid=${$.activityShopId}&sid=&un_area=`
            await member();
            await $.wait(2000)
            if ($.bean > 0) {
...
async function member() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            await $.wait(1000)
            await task('biouquan/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}`, 0, 1)
            // await task('biouquan/checkOpenCard', `pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&actorUuid=${encodeURIComponent($.actorUuid)}`)
        }
    }
}
                        ...
                        if (data.result) {
                            switch (function_id) {
                                ...
                                case 'wxActionCommon/getUserInfo':
                                    break;
                                case 'taskact/common/getDrawRecordHasCoupon':
                                    console.log(data)
                                    break;
                                case 'common/getSystime':
                                    console.log(data)
                                    break;
                                case 'biouquan/activityContent':
                                    // console.log(data)
                                    if (!data.data.hasEnd) {
                                        if ($.index === 1) {
