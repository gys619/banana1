...
!(async () => {
    ...
    authorCodeList = await getAuthorCodeList('https://gitee.com/444444521/JD-Scripts/raw/master/shareCodes/opencard102.json')
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/divi/active/activity/${$.authorNum}?activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&adsource=null&shareuserid4minipg=${encodeURIComponent($.secretPin)}&shopid=${$.activityShopId}&sid=&un_area=`
            await member();
            // await $.wait(5000)
            if ($.bean > 0) {
...
async function member() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            if ($.index === 1) {
                await task('divi/active/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}`, 0, 1)
            } else {
                await task('divi/active/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}`)
            }
            $.log("关注店铺")
            ...
            await task('divi/active/checkOpenCard', `pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&actorUuid=${encodeURIComponent($.actorUuid)}`)
            $.log("加入店铺会员")
