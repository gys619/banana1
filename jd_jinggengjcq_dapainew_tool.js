...
let actId = process.env.DAPAINEW || '';    //是否清空，如果为false，则上面设置了多少就只删除多少条
!(async () => {
    if (!cookiesArr[0]) {
...
async function bindWithVender(venderId) {
    let bodyStr = JSON.stringify({ "venderId": venderId, "bindByVerifyCodeFlag": 1, "registerExtend": {}, "writeChildFlag": 0, "channel": 401 });
    const req = {
    ...
    let opt = {
        url: `https://api.m.jd.com/client.action?appid=jd_shop_member&functionId=bindWithVender&body=${encodeURIComponent(bodyStr)}&client=H5&clientVersion=9.2.0&uuid=88888&h5st=${encodeURIComponent(h5st)}`,
        headers: {
            Host: 'api.m.jd.com',
            Accept: '*/*',
            Connection: 'keep-alive',
