...
[task_local]
#京喜财富岛
1 * * * * https://raw.githubusercontent.com/123/sync/jd_scripts/jd_cfd.js, tag=京喜财富岛, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jxcfd.png, enabled=true

================Loon==============
[Script]
cron "1 * * * *" script-path=https://raw.githubusercontent.com/123/sync/jd_scripts/jd_cfd.js,tag=京喜财富岛

===============Surge=================
京喜财富岛 = type=cron,cronexp="1 * * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/123/sync/jd_scripts/jd_cfd.js

============小火箭=========
京喜财富岛 = type=cron,script-path=https://raw.githubusercontent.com/123/sync/jd_scripts/jd_cfd.js, cronexpr="1 * * * *", timeout=3600, enable=true
 */
...
!(async () => {
  ...
  let res = await getAuthorShareCode('https://raw.githubusercontent.com/123/updateTeam/master/shareCodes/cfd.json')
  if (!res) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/123/updateTeam@master/shareCodes/cfd.json'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/123/updateTeam@master/shareCodes/cfd.json')
  }
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://123/cfd`, timeout: 30 * 1000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
...
function uploadShareCode(code) {
  return new Promise(async resolve => {
    $.post({url: `https://123/upload/cfd?code=${code}&ptpin=${encodeURIComponent(encodeURIComponent($.UserName))}`, timeout: 30 * 1000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
