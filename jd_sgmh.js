...
.finally(() => $.done())
//获取活动信息
function GetSharedCode(timeout = 0) {
				...
				try {
					...
					for (let i = 0; i < data.data.result.taskVos.length; i++) {
						if (data.data.result.taskVos[i].taskName === '邀请好友助力') {
							console.log(`【京东账号${$.index}（${$.UserName}）的${$.name}好友互助码】${data.data.result.taskVos[i].assistTaskDetailVo.taskToken}\n`);
							myInviteCode = data.data.result.taskVos[i].assistTaskDetailVo.taskToken;
							$.newShareCodes.push(myInviteCode);
						}
					}
				} catch (e) {
					$.logErr(e, resp);
				...
				try {
					...
					if (data.data.bizCode === 0) {
						...
						if (data.data.result.raiseInfo)
							scorePerLottery = parseInt(data.data.result.raiseInfo.nextLevelScore);
